...
import { getInstalledMcps, getRawMcps, installMcp, uninstallMcp, updateMcpTools } from '@liate/store';
import { probeMcpTools, executeMcpTool, createStreamDispatcher } from '@liate/runtime';
...
export function createMcpStreamableHandler(wsHub: WSHub) {
  ...
  return async (c: Context) => {
    ...
    if (method === 'tools/list' || method === 'initialize') {
      ...
      const allTools: any[] = [];
      for (const [sName, cfg] of Object.entries(installed as Record<string, any>)) {
